...
const yandexKey = 'trnsl.1.1.20190320T153212Z.9abc2cfea4d80821.f1fbfc211ba2b1dd9c466e06700e74ef118bff6c';
...
window.addEventListener('load', () => {
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(position => {
                ...
                preTranslate(userLang);

                // get greeting
                getGreeting(createGreeting(), userLang);

                // check language abbreviations for OpenWeatherMap API
                userLang = abbreviation(userLang);

                const key = '200de6c63566b3052efa4017421cb685';
                const url = `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${long}&lang=${userLang}&units=metric&APPID=${key}`;
...
function preTranslate(lang) {

    // check local storage and language
    if (!checkLocalStorage() && lang != defaultLanguage) {

        const greetings = 'Good Morning,Good Afternoon,Good Evening';
        const url = `${proxy}https://translate.yandex.net/api/v1.5/tr.json/translate?key=${yandexKey}&text=${greetings}&lang=${lang}`;
...
function getGreeting(greeting, lang) {
    ...
    if (checkLocalStorage()) {
        ...
        const url = `${proxy}https://translate.yandex.net/api/v1.5/tr.json/translate?key=${yandexKey}&text=${greeting}&lang=${lang}`
